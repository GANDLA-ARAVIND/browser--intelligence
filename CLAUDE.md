...
1. **Empty state — resolved, after the original hypothesis was measured and disproven.** Cosine always returns `k` results, so the UI cannot tell "no matches" from "your matches" by count alone; an absolute score floor was already ruled out (measured top scores ranged 0.376–0.892 across five queries, the transferability bug of §14 again). This section previously proposed a *shape*-based rescue instead — "a high top score with a steep drop-off means a real hit, a flat profile means nothing matched" — and v1.1 item 3 measured it directly rather than building it: 10 queries with real corpus content plus 5 deliberately absent topics, scored on top1, gap-to-2, gap-to-5, and stdev of the top 20. **The hypothesis is disproven, not merely unbuilt:** the steepest drop-off of all 15 queries belongs to an absent topic (`"tax filing deadline"`, top1 0.600, gap-to-2 0.265 — the exact "confident" shape the hypothesis describes, and a false positive, matching only on the word "deadline"), and no one of the four metrics separates the two groups by range — `"react state management"`, genuinely present at 0.464, scores *below* that absent false positive (DECISIONS.md has the full table). No cutoff on any of these signals clears both cases correctly, so neither a hard threshold nor a "nothing closely matched" heading is supportable — either would assert a discrete judgement the data cannot back up, which is §2's no-invented-metrics problem in a new form. **Shipped instead: continuous visual degradation, not a binary decision.** `top1` alone scales result-card opacity in `Search.tsx` — clamped so results stay legible even at the floor — with no label, no invented percentage, and no heading change; a weak result set looks visibly less confident without the UI ever claiming to have decided "nothing matched." The existing "ranks, not matches" copy (§15) still carries the honesty in words; the gradient carries it visually. The one narrower case that *is* honest to report outright — zero results because an active **filter** excluded everything — remains distinguished from this and shown as its own message, since that is a fact about set membership, not a relevance judgement.
...
**A Time surface (v2 — specified here, not started).** Answers factual
questions about attention, never evaluative ones: where did my time go today
/ this week / this month, which topics consumed the most attention, what
kinds of content (format) did time go to, which pages did I return to most,
and how has this changed over time. **Explicitly not**: productivity scores,
productive-vs-distracting splits, goal progress, streaks — §2's
no-invented-metrics rule covers every one of these the same way it covers
everything else this product refuses to fabricate. The user draws
conclusions; the product states facts. This partially overlaps the "Patterns
view" line above — Time is the fuller, concrete spec for the
attention-by-topic and attention-by-format half of that idea; the
hour-by-weekday heatmap and session-length distribution remain a separate,
still-unspecified piece of Patterns.

*Architecture: a view, not a layer.* No new capture path, no new store, no
new pipeline stage. Every field it needs already exists — `activeSeconds`,
`clusterId`, `visitCount`, `lastVisit` and `format` on `PageRecord` (§4), and
`label` on `ClusterRecord` (§5, §6). Planned as two files: `src/lib/analytics.ts`
(pure aggregation, isomorphic, the same discipline every other `lib/` module
already holds to) and `src/dashboard/TimePage.tsx` (the surface itself,
rendering only). If this design is ever found to need a parallel capture path
or a second store, that is a signal something upstream is wrong, not a sign
this feature needs more infrastructure.

*Division of labour, worth recording explicitly.* Intelligence (§5, §6, and
this section's own Search and Topics surfaces) answers "what did I consume,
and where is it" via embeddings, clustering and derived labels. Analytics
answers "where did my time go" by aggregating the exact same records a
different way. They share one taxonomy — that sharing is the differentiator,
not a coincidence of implementation. A screen-time tool that only counts
domains and durations cannot say "Algorithms — 1h 40m this week," because it
has no notion of what a page was actually about; this product already builds
that notion for Search and Topics, and Time is free to reuse it rather than
re-deriving anything.

*Hard constraint, to stay prominent wherever this gets built:* `activeSeconds`
is populated only for live-captured pages — §9 measures engagement live and
explicitly refuses to infer it — roughly 40 of 5,100 pages in the corpus as
of this writing. Time therefore covers **captured pages only**, and the
surface must say so as plainly as Today's reconstructed-session banner states
its own equivalent limitation (§15) — never silently. Backfilled history
contributes to Search and Topics; it contributes nothing to Time, because
there is no engagement signal to aggregate for a page whose only timestamp is
a last-touch reconstruction, not a record of active attention.

*Build order: do not start until 2–3 weeks of real capture have
accumulated.* Building or tuning this surface against ~40 captured pages
would validate nothing — the same too-small-a-fixture caution that already
shaped the Phase 2 embedding-cap and quality-threshold measurements (§11,
DECISIONS.md).

*A possible later addition, not promised.* `Intent` (§4) is already typed —
`'learning' | 'debugging' | 'job-hunting' | 'shopping' | 'entertainment' |
'reference'` — but unpopulated. It would stay descriptive rather than
evaluative in the same spirit as the rest of this surface: "6 hours
job-hunting" is a fact, "6 hours unproductive" is a judgement this product
does not make. Inferring it reliably is unmeasured, so it is recorded here as
a candidate, not committed to.
...
| Rule | Why it binds |
|---|---|
| **Proper-noun dominance does not generalize as originally measured — confirmed narrowly for cluster #1's title format, unconfirmed as the unified mechanism first proposed. Do not pursue a third mitigation without a fresh diagnosis** | Originally measured on an earlier, smaller corpus snapshot with hand-picked titles: `Software Engineer - Bengaluru - Capgemini` ~ the same role in Hyderabad at Capgemini = **0.889**, against the same role at Infosys **0.775**, Microsoft **0.509**, UnitedHealth **0.510** — framed as one mechanism explaining both an over-split (job clusters fragmenting by employer) and cluster #1's over-merge. **A v1.1 remeasurement on the current, larger corpus does not reproduce the over-split half** (DECISIONS.md): two same-company pairs reconstructed from real current titles — Capgemini (**0.660**) and Optum (**0.488**) — do not clear same-role-different-company (**~0.71–0.84**, depending on candidate); Optum's pair scores *below* it. The over-merge half re-confirms independently: cluster #1 is still there (now 510 pages, 93% Google-Maps-direction titles), but its own composition rules out the general theory as the explanation — these titles carry **no company or employer name at all**, so the cause is place-name domination specific to one title *format* (Maps directions), not the broader "proper nouns dominate short titles" mechanism the original row proposed as unifying both symptoms. **Two mitigations were measured and rejected before this correction was written, not after** — the diagnosis was checked before more engineering went into fixing it (DECISIONS.md): (a) stripping a domain-derived suffix before embedding made cluster #1 *bigger* (592 pages), not smaller, and helped only one narrow pairwise case; (b) stripping non-initial capitalised tokens destroyed cluster #1 into noise rather than fixing it, regressed exact-match search (`Two Sum - LeetCode` loses its own match), and corrupted 51% of real Telugu titles despite being predicted a script-safe no-op — the sixth instance of the non-Latin standing rule below. **Do not attempt a third proper-noun mitigation without first establishing what cluster #1 and the fragmented job clusters actually have in common, if anything** — the evidence so far says they may not share one, and tuning `shared`/`knn` was already ruled out separately (see the `largest%` rule above) |
...
| **Standing rule: before trusting any frequency or rarity signal, verify which population it is actually computed over** | BM25's IDF is arithmetically correct and was still wrong: measuring `df("maps")` over `embedText` gave **5** of 5,871 (rare, heavily up-weighted) against the true **804** of 5,871 over raw `title` (common, correctly down-weighted) — not because the formula is broken, but because `embedText` had already been through the boilerplate-suffix stripper (`titles.ts`), which removes "Google Maps" from every title long enough to survive it (§14, DECISIONS.md). The stripper's own frequency threshold is correct for *its* job — it counts over raw titles, exactly as it should, to decide what is site-wide chrome. The error was reusing its output as input to a second, unrelated frequency-based signal, which silently inherited the first transform's population change. **This is not specific to BM25.** c-TF-IDF (`labels.ts`) deliberately counts document frequency *across clusters*, not across the whole corpus of pages — the right population for "is this term distinctive to this cluster," and worth restating as a deliberate choice rather than assumed correct by construction. `deriveBoilerplateSuffixes`'s own threshold (the mechanism above) counts over raw titles before anything else has touched them, which is exactly why it stayed correct while a downstream consumer of its output did not. **Before shipping or reusing any occurrence-counting signal — a rarity weight, a boilerplate-frequency threshold, a junk filter's occurrence floor — state which population it counts over, and confirm nothing upstream has already transformed that population for a different purpose** |
| **Standing rule: any string-based heuristic must be tested against Telugu, Chinese and Arabic before it ships** | Six instances so far, each of which would have silently deleted or misjudged non-English content: (1) the `\W`-based empty-title check — JS `\W` is ASCII-only, so every title written entirely in a non-Latin script matched "no word characters" and was dropped; (2) the bare-URL rule, whose first draft keyed on "no whitespace", which is *normal* in Chinese, Japanese and Thai; (3) the English stopword list in quality scoring, which had to be skipped rather than replaced with a neutral value, or every non-English page reads as navigation chrome. **(4)** the sentence-terminator class, which covered Latin, CJK and Devanagari but not Arabic/Urdu (U+061F, U+06D4) — and Thai, Lao, Khmer and Myanmar end sentences with a *space*, so zero terminator density there is normal prose. **(5)** the 400-*character* usable-content floor: the same paragraph is 123 characters in Chinese and 368 in English, a measured ~3× density difference, so a character floor rejects genuine CJK prose as too short. Replaced with script-neutral **units** (one CJK character, or one whitespace token elsewhere) — the word tokenizer had the mirror bug, matching an entire unspaced Chinese sentence as one token and inflating terminator density to 57 per 100 "words" against 6.7 for the same English. **(6)** a v1.1 embedding-text candidate (strip capitalised tokens outside sentence-initial position) predicted to be "a no-op outside Latin text, since capitalisation doesn't exist in most scripts" — wrong: 51% of real Telugu titles in the corpus were corrupted anyway, because they are YouTube titles **code-mixed** with English words ("Weekend", "Phoenix", "Los Angeles", "ONE Pan" all stripped from otherwise-Telugu titles). Synthetic pure-script Chinese and Arabic fixtures tested as true no-ops, exactly as predicted; synthetic **mixed**-script fixtures reproduced the same corruption the real Telugu titles showed. **A script heuristic tested only on pure-script fixtures is not tested** — code-mixing is the actual risk surface for anything keyed on a Latin-only signal (capitalisation, ASCII ranges, whitespace), and a single-script synthetic fixture cannot surface it; only real corpus text or a deliberately mixed-script fixture can. The failure mode is always the same and always silent. **Instances 4, 5 and 6 were caught by this rule during pre-ship testing rather than in production, which is the rule working.** Seven-script audit (instances 4–5) passes: Chinese 0.90, Telugu 0.86, Japanese 0.79, Hindi 0.77, Urdu 0.70, Arabic 0.60, Thai 0.45 — none rejected |
